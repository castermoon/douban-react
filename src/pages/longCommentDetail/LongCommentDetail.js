...
// import {SyntheticEvent} from "_@types_react@18.0.14@@types/react";
//
...
const LongCommentDetail = () => {
	...
	const respondList = useRef(null)
	const respondItemList = useRef([])
			...
			<BaseBody
				...
				left={
					<Fragment>
						...
						<ul className={style.respond_list} ref={respondList}>
							{
								longCommentDetailRes.length > 0 && longCommentDetailRes.map((item,index) => {
	...
	function createResRes(event,longCommentRes_id,respond_id,index){
		...
		//index表示被回复楼层所在列表的索引。
		let scrollTop = respondItemList[index].offsetTop + respondItemList[index].offsetHeight - 200
		axios.post('/api/longCommentDetail/createLongCommentResRes',{
			user_id:userInfo.userInfo.id,
			longCommentRespond_id:longCommentRes_id,
			longComment_id:longCommentDetail.id,
	...
	function getLi(index){
		console.log(respondItemList.current[index])
	}

	function getUl(){
		console.log(respondList.current)
	}
