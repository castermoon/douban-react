...
class Celebrity extends Component{
	...
	componentDidMount(){
		axios.get('/api/celebrity/'+this.props.match.params.celebrity_id)
			.then((res) => {
				if(res.data.errno == 0){
					this.setState((prevState) => {
						return {
							celebrityData:res.data.data.celebrityData,
							recentWorksListData:res.data.data.recentWorksListData
						}
					})
				}
			})
	...
	render() {
		const {celebrityData,recentWorksListData} = this.state
		return(
			<Fragment>
				<CommonHeader />
				<BaseBody
